...
function verificarAcesso(event) {
    ...
    if (tentativasErradas >= limiteTentativas) {
        ...
        const relogio = setInterval(() => {
            segundosRestantes--
            mensagem.textContent = `Muitas tentativas! Sistema bloquedo por ${segundosRestantes} segundos.`
            ...
            if(segundosRestantes <= 0){
                ...
                mensagem.style.color = "white"
                mensagem.textContent = "Sistema desbloqueado. Tente novamente."
            }
        },1000)// Atualiza a cada segundo
        return
    } else {
        mensagem.style.color = "yellow"
        mensagem.textContent = `Usuário ou senha incorreta! (${tentativasErradas}/${limiteTentativas})`
    }
}
